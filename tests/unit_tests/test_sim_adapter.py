...
Layered so most run with no extra deps:

* Adapter-shape tests need only skidl (assert the ``_FlatCircuit`` view exposes
  exactly what the converter reads).
* Conversion tests need PySpice + circuit_synth (skidl-authored circuit ->
  correct SPICE netlist through the REAL cs ``SpiceConverter``).
* Live tests additionally need ngspice (skidl-authored circuit actually
  simulates: DC op point, and a diode places with correct pin-name polarity).
...
try:
    from circuit_synth.simulation.converter import SpiceConverter  # noqa: F401
...
if HAS_CS_SIM:
    try:
        import circuit_synth.simulation.simulator  # noqa: F401  configures ngspice

        _probe = SpiceConverter  # sentinel; real probe below in a fixture
        HAS_NGSPICE = True  # optimistic; live tests still guard per-run
    except Exception:
        HAS_NGSPICE = False

requires_cs_sim = pytest.mark.skipif(
    not HAS_CS_SIM, reason="circuit_synth.simulation (PySpice) not installed"
)
...
def test_ldo_macromodel_regulates(tmp_path):
    ...
    spice = SpiceConverter(skidl_flat_view()).convert(strict=True)
    try:
        import circuit_synth.simulation.simulator  # noqa: F401
