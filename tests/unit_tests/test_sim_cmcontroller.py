...
def test_cmcontroller_emits_pwm_engine_and_buck_stage():
    ...
    # gm error amp into the real external VC net (28.D cell): I into VC = GM*(VREF-FB)
...
def test_soft_reference_ramp_emitted_only_when_tss_positive():
    ...
    assert "BU1_ea 0 VC I = 0.00025*(0.8 - V(FB))" in net_hard
    ...
    assert "BU1_ea 0 VC I = 0.00025*(V(U1_vref) - V(FB))" in net_soft, net_soft


@requires_sim
def test_negative_vref_accepted():
    """VREF may be negative (an inverting FB configuration), unlike the voltage-mode
    buck which requires a positive VREF."""
    _setup()
    u = _cmc_part(
