    ...
    **options,
    ...
    for attempt in range(retries):
        ...
        try:
            ...
            # Stage-24 wired mode: snap BEFORE route (parts moved to final
            # positions, NOT stubbed) so the per-net A* router wires every net on
            # the final geometry -- pin-touched pairs route to nothing, spread
            # nets get real wires. Classic order (route then snap) is the default.
            if options.get("snap_before_route", False) and options.get(
                "auto_stub", False
            ):
                _snap_two_pin_parts(node, stub=False)
            node.route(**options)
        ...
        if options.get("auto_stub", False) and not options.get(
